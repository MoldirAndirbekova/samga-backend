...
class FruitSlicerGameState:
    ...
    def get_game_state(self):
        """Return game state data instead of rendered image"""
        print(f"Getting game state: active={self.game_active}, score={self.score}, fruits={len(self.fruits)}")
        
        # Prepare fruits data for client rendering
        fruits_data = []
        for fruit in self.fruits:
            fruits_data.append({
                "id": fruit.id,
                ...
                "velocityX": fruit.velocity_x,
                "velocityY": fruit.velocity_y,
                "sliced": fruit.sliced,
                "slicedByPlayer": fruit.sliced_by_player,
                "age": fruit.age,
                "isBomb": fruit.is_bomb,
                "type": fruit.id % 5,  # Fruit type for rendering
                "createdAt": fruit.created_at.timestamp() * 1000  # Convert to JS timestamp
            })
        
        # Prepare blade trail data
        blade_trail_data = []
        for point in self.blade_trail:
            blade_trail_data.append({
                "x": point["x"],
                "y": point["y"],
                "time": point["time"].timestamp() * 1000  # Convert to JS timestamp
            })
        
        # Prepare nose position data
        nose_data = None
        if self.nose_position:
            nose_data = {
                "x": self.nose_position["x"],
                "y": self.nose_position["y"]
            }
        ...
        return {
            ...
            "difficulty": self.difficulty,
            "fruits": fruits_data,
            "bladeTrail": blade_trail_data,
            "nosePosition": nose_data,
